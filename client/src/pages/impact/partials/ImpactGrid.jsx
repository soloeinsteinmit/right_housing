...
const NoResultsAnimation = React.memo(() => (
  <motion.div initial="hidden" animate="visible" className="w-64 h-64 mx-auto">
    <motion.svg
...
const SearchSection = React.memo(({ 
  ...
  >
    ...
    <p className="text-lg text-gray-600 mb-8">
      Have you heard about the lives we've touched? Discover inspiring stories of
      hope, resilience, and transformation in our community.
    </p>
    ...
    <div className="relative max-w-xl mx-auto">
      ...
      {searchQuery && (
        <div className="mt-4 text-sm text-gray-600">
          Found {filteredStoriesCount}{" "}
          {filteredStoriesCount === 1 ? "story" : "stories"}
        </div>
      )}
    </div>
  </motion.div>
...
const StatBadge = React.memo(({ icon: Icon, label, value }) => (
...
const StoryCard = React.memo(({ story, index }) => {
  const getIconForStat = (key) => {
    if (key.includes("Size") || key.includes("Helped")) return Users;
    if (key.includes("Success") || key.includes("Activities")) return Target;
    return Clock;
  };
...
export default React.memo(ImpactGrid);
